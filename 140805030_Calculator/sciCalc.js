...
function factorial(exclam) {
    if (Number.isInteger(exclam)) {
        if (exclam < 2) return 1 ;
        return exclam * factorial(exclam - 1) ;
    }
}
...
function trigo(hat) {
    ...
    x.value += hat + "(" ;
    y.innerHTML += (/[\d)IE]/.test(y.innerHTML.slice(-1)))?
    " * Math." + hat + "(Math.PI / 180 * " : "Math." + hat + "(Math.PI / 180 * ";
}

function trigo1(hats) {
    var x = document.getElementById("result");
    var y = document.getElementById("myPara");
    x.value += hats + "\u207B\u00B9("
    y.innerHTML += (/[\d)IE]/.test(y.innerHTML.slice(-1)))?
    " * 180 / Math.PI * Math.a" + hats + "(" : "180 / Math.PI * Math.a" + hats + "(";
}
